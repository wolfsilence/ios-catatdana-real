...
final class CDHostViewModel {
    ...
    var transactions: [EntityTrade] = []
    var reminders: [EntityReminder] = []
    var creditCards: [EntityBankCard] = []

    // MARK: - Tab

    var selectedTab: MainTab = .home

    var recentTransactions: [EntityTrade] {
    ...
    func refreshData() {
        ...
        creditCards = DatabaseHelper.shared.loadCreditCards()
        calculateSummary()
    }
    ...
    private func calculateSummary() {
        ...
        totalIncome = monthTx.filter { $0.type == .income }.reduce(0) { $0 + $1.num }
        totalExpense = monthTx.filter { $0.type == .expense }.reduce(0) { $0 + $1.num }
    ...
    func submitBiz(type: String, data: [String: String]) async {
        let req = Entity20(pclb: type, qkipkeyov: data)
