...
struct CDRecordTransactionView: View {
    ...
    var body: some View {
        ...
        .toast(isPresented: $showSavedToast, message: AllStr.recordTransactionSavedToast)
        ...
        .confirmationDialog(AllStr.recordTransactionPhotoTitle, isPresented: $showPhotoAction) {
    ...
    private var pageHeader: some View {
        HStack(spacing: 12) {
            ...
            Text(AllStr.recordTransactionTitle)
        ...
        HStack(spacing: 0) {
            ForEach(TransactionType.allCases, id: \.self) { t in
                Button {
                    ...
                    Text(t == .expense ? AllStr.commonExpense : AllStr.commonIncome)
    ...
    private var categoryGrid: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(AllStr.recordTransactionCategory)
                .font(.system(size: 13))
                .foregroundColor(Colors.textSecondary)
    ...
    private var locationAndNote: some View {
        VStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 8) {
                    Image(systemName: "location.fill")
                        .font(.system(size: 14))
                        .foregroundColor(Colors.primary)
                    TextField(AllStr.recordTransactionLocationPlaceholder, text: $vm.location)
                        .font(.system(size: 14))
                }
                .padding(12)
                .background(Colors.launchBackground)
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text(AllStr.commonNote)
                    .font(.system(size: 13, weight: .medium))
                    .foregroundColor(Colors.textSecondary)
                TextField(AllStr.recordTransactionNotePlaceholder, text: $vm.note, axis: .vertical)
                    .font(.system(size: 14))
                    .padding(12)
                    .background(Colors.launchBackground)
                    .clipShape(RoundedRectangle(cornerRadius: 12))
    ...
    private var photoSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(AllStr.recordTransactionPhotoTitle)
                .font(.system(size: 13))
                .foregroundColor(Colors.textSecondary)
            ...
            if let image = capturedImage {
                ...
                HStack {
                    Image(systemName: "photo.fill")
                        .foregroundColor(Colors.primary)
                    Text(AllStr.recordTransactionPhotoAttached)
                        .font(.system(size: 13))
                        .foregroundColor(Colors.textPrimary)
                    Spacer()
                ...
                Button {
                    ...
                    VStack(spacing: 8) {
                        Image(systemName: "camera.fill")
                            .font(.system(size: 24))
                            .foregroundColor(Colors.primary)
                        Text(AllStr.recordTransactionTakePhoto)
                            .font(.system(size: 13, weight: .medium))
                            .foregroundColor(Colors.primary)
                    }
                    .frame(maxWidth: .infinity)
    ...
    private var saveButton: some View {
        Button {
            ...
            Text(vm.saved ? "✓ \(AllStr.commonSaved)" : AllStr.recordTransactionSave)
