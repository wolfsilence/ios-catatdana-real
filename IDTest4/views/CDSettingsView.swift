...
struct CDSettingsView: View {
    ...
    private var appVersionCdk: String {
    ...
    var body: some View {
        ZStack {
            ...
            // 确认弹窗
            if vm.showLogoutConfirm { logoutOverlayCdk }
            if vm.showDeleteConfirm { deleteOverlayCdk }
            if vm.showDeleteSecondConfirm { deleteSecondOverlayCdk }
        }
    ...
    private var appInfoSectionCdk: some View {
        VStack(alignment: .leading, spacing: 0) {
            sectionHeaderCdk(AllStr.stAi)

            VStack(spacing: 0) {
                Button {
                    Task { await vm.checkVersion() }
                } label: {
                    infoRowCdk(AllStr.stV, appVersionCdk)
                }
                .buttonStyle(.plain)
            }
    ...
    private var deleteOverlayCdk: some View {
        ZStack {
    ...
    private var dangerZoneCdk: some View {
        VStack(alignment: .leading, spacing: 0) {
            sectionHeaderCdk(AllStr.stDz)
    ...
    private func infoRowCdk(_ label: String, _ value: String) -> some View {
        HStack {
            Text(label)
                .font(.system(size: 14))
                .foregroundColor(AppColors.strSecondary)
    ...
    private var deleteSecondOverlayCdk: some View {
        ZStack {
            Color.black.opacity(0.4).ignoresSafeArea()
    ...
    private func dangerButtonCdk(icon: String, iconBg: Color, iconColor: Color, label: String, labelColor: Color, action: @escaping () -> Void) -> some View {
    ...
    private var logoutOverlayCdk: some View {
        ZStack {
            Color.black.opacity(0.4).ignoresSafeArea()
    ...
    private func sectionHeaderCdk(_ title: String) -> some View {
        Text(title)
