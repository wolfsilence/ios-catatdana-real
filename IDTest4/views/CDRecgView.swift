...
struct CDRecgView: View {
    ...
    var body: some View {
        ...
        .alert(AllStr.pmCt, isPresented: $showSettingsAlert) {
            Button(AllStr.pmC) {
                didReportResult = true
                onResult(nil)
                dismiss()
...
private struct LiveDetectionWebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        progressView.progressTintColor = UIColor(AppColors.primary)
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
        ...
        func userContentController(
            ...
            Task { [weak self] in
                let result: NetResp<Entity1> = await Net.shared.post(
                    path: Paths.pvyft,
                    encodableBody: req
                )
                await MainActor.run {
