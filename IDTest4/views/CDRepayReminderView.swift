...
struct CDRepayReminderView: View {
    ...
    private func reminderCard(_ r: EntityReminder) -> some View {
    ...
    private var addForm: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            formField(AllStr.rmNl, placeholder: AllStr.rmNp, text: $vm.name)
            formField(AllStr.rmAl, placeholder: AllStr.rmAp, text: $vm.amount, keyboard: .numberPad)
            DatePicker(AllStr.rmDl, selection: $vm.dueDate, in: Date()..., displayedComponents: .date)
                .font(.system(size: 13))
                .foregroundColor(AppColors.strSecondary)
            formField(AllStr.rmNte, placeholder: AllStr.rmNtp, text: $vm.note)
