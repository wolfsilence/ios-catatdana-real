...
struct CDMainView: View {
    ...
    private var balanceCard: some View {
        Button {
            ...
            VStack(spacing: 0) {
                HStack {
                    Text(Strings.Home.monthlyBalance)
                        .font(.system(size: 13))
                        .foregroundColor(.white.opacity(0.8))
                    Spacer()
                    HStack(spacing: 4) {
                        Text(Strings.Home.viewAnalysis)
                            .font(.system(size: 12))
                            .foregroundColor(.white.opacity(0.8))
                ...
                HStack(spacing: 24) {
                    summaryItem(icon: "arrow.up", label: Strings.Common.income, amount: vm.totalIncome)
                    summaryItem(icon: "arrow.down", label: Strings.Common.expense, amount: vm.totalExpense)
                }
                .padding(.bottom, 12)
    ...
    private var featureGrid: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(Strings.Home.mainFeatures)
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if vm.recentTransactions.isEmpty {
                VStack(spacing: 8) {
                    ...
                    Text(Strings.Home.noTransactions)
                        .font(.system(size: 14))
                        .foregroundColor(Colors.textSecondary)
                }
                .frame(maxWidth: .infinity)
                ...
                Button {
                    ...
                    VStack(spacing: 4) {
                        Image(systemName: tab == vm.selectedTab ? "\(tab.icon)" : tab.icon)
                            .font(.system(size: 20))
                            .foregroundColor(tab == vm.selectedTab ? Colors.primary : Colors.textHint)
                        Text(tab.label)
    ...
    private func featureOverlay(for feature: MainFeature) -> some View {
        switch feature {
        case .record:
            CDRecordTransactionView(onBack: { activeFeature = nil }, onSaved: { vm.refreshData() })
        case .reminder:
            CDPaymentReminderView(onBack: { activeFeature = nil })
        case .creditcard:
            CDCreditCardBindingView(onBack: { activeFeature = nil })
        case .emi:
            CDEMICalculatorView(onBack: { activeFeature = nil })
        case .maxloan:
            CDMaxLoanCalculatorView(onBack: { activeFeature = nil })
        case .exchange:
            CDExchangeRateView(onBack: { activeFeature = nil })
        case .analysis:
        ...
        case .contact:
            CDContactView(onBack: { activeFeature = nil })
        }
    }
