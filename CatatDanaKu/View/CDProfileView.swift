...
struct CDProfileView: View {
    ...
    var body: some View {
        ...
        .padding(.top, 8)
        .onAppear {
            localNickname = UserDefaults.standard.string(forKey: Keys.profileNickname) ?? ""
        }
        // Camera
        ...
        // Name alert
        .alert(Strings.Profile.changeNameTitle, isPresented: $showNameAlert) {
            TextField(Strings.Profile.namePlaceholder, text: $nameInput)
            ...
            Text(Strings.Profile.nameMessage)
        }
    }
    ...
    private var illustrationHeader: some View {
                ...
                HStack(alignment: .bottom, spacing: 12) {
                    VStack(spacing: -12) {
                        Circle().fill(Color(hex: "#E8F8EE")).frame(width: 40, height: 40)
                            .overlay(Text("Rp").font(.system(size: 10, weight: .bold)).foregroundColor(Colors.primary))
                        Circle().fill(Color(hex: "#1BC459").opacity(0.5)).frame(width: 40, height: 40)
                        Circle().fill(Colors.primary).frame(width: 40, height: 40)
                    }
                    ...
                    ZStack {
                        ...
                        Circle().fill(Colors.primary).frame(width: 22, height: 22)
                    }
                    .offset(y: -10)
                }
                .offset(y: -20)
        ...
        .background(
            LinearGradient(
                colors: [Color(hex: "#E8F8EE"), Colors.launchBackground],
                startPoint: .top,
                endPoint: .bottom
            )
        )
    ...
    private var avatarAndInfo: some View {
        VStack(spacing: 8) {
            Button { showAvatarSheet = true } label: {
                ...
                .shadow(color: Colors.primary.opacity(0.3), radius: 4, y: 2)
                .overlay(alignment: .bottomTrailing) {
                    ZStack {
                        Circle().fill(Color.white).frame(width: 22, height: 22)
                        Circle().fill(Colors.primary).frame(width: 18, height: 18)
            ...
            .offset(y: -32)
            .padding(.bottom, -32)
            .confirmationDialog(Strings.Profile.avatarTitle, isPresented: $showAvatarSheet) {
    ...
    private var statsBar: some View {
        HStack(spacing: 0) {
            statItem("\(transactionsCount)", Strings.Profile.transactions)
            Rectangle().fill(Color.black.opacity(0.06)).frame(width: 1, height: 32)
            statItem("\(cardsCount)", Strings.Profile.creditCards)
            Rectangle().fill(Color.black.opacity(0.06)).frame(width: 1, height: 32)
            statItem("\(remindersCount)", Strings.Profile.reminders)
        }
        .padding(16)
